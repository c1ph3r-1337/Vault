i have a idea that where some people on the decentralized network , have a thing to computationally or any gig , after the computation  or completing the gig, the person would be rewarded a coin from coin base(token) 

find the condition to solve an AI  and decide what value should be given to the person and then if the the person decides the value of the coin is provided less then he can put it in review and people on network get to decide the value to be given 
...
also for the flexibility of the askers the , the value of the gig should be decided in dollar as the tokens value may increase or decrease so the token payment might sometimes become biased (expensive or cheap sometimes) but the payment should be done in coins only
  ...
  - token approval for worker stake
  - acceptJob
  - token approval for validator stake
  - stakeAsValidator
  - vote
  - finalizeValidation
  - finalize
    
      Use flow on the worker tab:

  1. Approve stake
  2. Accept funded job
  3. Approve validator stake
  4. Stake as validator
